...
ModelData ObjLoader::loadObj(const std::string& filename) {
...
float ObjLoader::findFurthestPoint(const std::vector<Vertex>& vertices) {
...
void ObjLoader::processVertex(const std::vector<std::string>& vtxData, std::vector<Vertex>& vertices, std::vector<GLuint>& indices) {
...
void ObjLoader::handleProcessedVertex(Vertex* previousVertex, GLuint newTextureIndex, GLuint newNormalIndex, std::vector<GLuint>& indices, std::vector<Vertex>& vertices) {
...
std::string ObjLoader::readObjFile(const std::string& filename) {
  std::ifstream file(filename.c_str());
...
std::vector<std::string> ObjLoader::splitLineBySpace(const std::string& line) {
  return splitLine(line, ' ');
}

std::vector<std::string> ObjLoader::splitLine(const std::string& line, char delim) {
  std::stringstream ss;
  ss.str(line);
  std::string item;
...
bool ObjLoader::lineStartsWith(const std::string& line, const std::string& subStr) {
  return 0 == std::strncmp(line.c_str(), subStr.c_str(), std::strlen(subStr.c_str()));
}

glm::vec2 ObjLoader::parsePair(const std::vector<std::string>& line) {
  return glm::vec2{std::atof(line[1].c_str()), std::atof(line[2].c_str())};
}

glm::vec3 ObjLoader::parseTriplet(const std::vector<std::string>& line) {
